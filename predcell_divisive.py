...
class ErrorUnit(nn.Module):
    ...
    def forward(self, state, recon):
        ...
        eps = 1e-3
        self.TD_err = torch.div(recon,torch.clamp(state, min = eps, max = 1e30))
        PE = torch.div(state,torch.clamp(recon, min = eps, max = 1e30))
        self.BU_err = self.W(PE.float())
            ...
            for lyr in range(self.num_layers):
                ...
                if iternumber <= 1000:
                    if lyr == 0:
                        ...
                        loss += torch.sum(torch.abs(self.err_units[lyr].TD_err + self.err_units[lyr-1].BU_err))*(lambda1**(lyr))
                if iternumber > 1000:
                    if lyr == 0:
                        # assign a bit less importance to higher layers
                        loss += torch.sum(torch.abs(self.err_units[lyr].TD_err))*(lambda2**(lyr))
                    else:
                        loss += torch.sum(torch.abs(self.err_units[lyr].TD_err + self.err_units[lyr-1].BU_err))*(lambda2**(lyr))
