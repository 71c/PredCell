...
for epoch in range(num_epochs):
    for idx, sentence in enumerate(x[:100]):
        ...
        optimizer.step()
        for i,j in enumerate(trainable_err_params[:-2]): 
            writer.add_histogram('error_weights'+str(i), j,global_step = step)
            writer.add_histogram('error_grads'+str(i), j.grad,global_step = step)
        for i,j in enumerate(trainable_st_params[6:]):
            writer.add_histogram('st_weights'+str(6+i), j,global_step = step)
            writer.add_histogram('st_grads'+str(6+i), j.grad,global_step = step)
